...
from losses_bnulihaixia import BinomialLoss
import numpy as np
import pdb
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/Binomial_ckpt.t7')
        best_acc = acc
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        if os.path.isfile('./checkpoint/Binomial_ckpt.t7'):
            checkpoint = torch.load('./checkpoint/Binomial_ckpt.t7')
    ...
    criterion = BinomialLoss(alpha=1,beta=1)
