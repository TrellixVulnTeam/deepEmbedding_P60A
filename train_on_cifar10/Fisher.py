...
class FisherLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # inter_base=torch.mean(inter_loss)
        # intra_base=torch.mean(intra_loss)

        intra_logit = torch.sum(torch.exp(self.margin*(intra_loss)))
        inter_logit = torch.sum(torch.exp(self.margin*(inter_loss)))
        
        loss=-torch.log(inter_logit/(inter_logit + intra_logit))
