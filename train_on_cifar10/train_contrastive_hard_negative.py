...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/contrastive_hard_negative_ckpt.t7')
        best_acc = acc
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        if os.path.isfile('./checkpoint/contrastive_hard_negative_ckpt.t7'):
            checkpoint = torch.load('./checkpoint/contrastive_hard_negative_ckpt.t7')
    ...
    criterion = OnlineContrastiveLoss(margin=1,pair_selector=HardNegativePairSelector())
