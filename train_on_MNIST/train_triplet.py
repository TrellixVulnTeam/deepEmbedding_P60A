...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        loss,len_triplets = criterion(embeddings, targets)
        if len_triplets==0:
            continue
        loss.backward()
        optimizer.step()
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/triplet_ckpt.t7')
        best_acc = acc
...
if __name__ == '__main__':
    ...
    parser.add_argument('--partition-proportion', default=7, type=int, help='partition proportion')
    ...
    criterion = OnlineTripletLoss(margin=1,triplet_selector=AllTripletSelector())
