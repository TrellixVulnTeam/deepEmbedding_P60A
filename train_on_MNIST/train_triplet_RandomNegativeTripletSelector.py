...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/triplet_RandomNegativeTripletSelector_ckpt.t7')
        best_acc = acc
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        if True:
            checkpoint = torch.load('./checkpoint/triplet_RandomNegativeTripletSelector_ckpt.t7')
