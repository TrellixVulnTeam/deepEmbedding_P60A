...
from FisherCriterion import FisherLoss
import numpy as np
import pdb
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/Fisher2_ckpt.t7')
        best_acc = acc
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        if os.path.isfile('./checkpoint/Fisher2_ckpt.t7'):
            checkpoint = torch.load('./checkpoint/Fisher2_ckpt.t7')
    ...
    criterion = FisherLoss(margin=1,hard_mining=True)
